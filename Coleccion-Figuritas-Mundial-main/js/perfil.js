...
let figuritas = [
   ...
   figuritas = figuritas.filter(figurita => idfiguritas.includes(figurita.id)); 

  function mostrarFiguritas(lista) {
    const contenedorFig = document.querySelector('.contenedor-figuritas-favoritas');
     lista.slice(0, 3).forEach(figurita => {
        const imagen = document.createElement('img');
        imagen.classList.add('figurita-favorita');
imagen.src = figurita.imagen;
imagen.alt = figurita.alt;
    contenedorFig.appendChild(imagen);
...
if(figuritas.length==0){
    ...
    texto.appendChild(document.createTextNode("Todavia no elegiste ninguna figurita como favorita"));
    const contenedorFig = document.querySelector('.contenedor-figuritas-favoritas');
    contenedorFig.appendChild(texto);
}
else{
